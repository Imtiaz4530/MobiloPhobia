...
export default function Home({ brands, products }) {
  const { brands: brandStore, products: productStore } = useStoreActions(
    (action) => action
  );
  const {
    brands: { data: brandData, loading: brandLoading, error: brandError },
    products: {
      data: productData,
...
export async function getServerSideProps() {
  const res = await fetch(
    "http://localhost:1337/api/brands?populate=*&pagination[start]=0&pagination[limit]=10"
  );
  const res2 = await fetch(
    "http://localhost:1337/api/products?populate=*&pagination[start]=0&pagination[limit]=100"
  );
  const res3 = await fetch(
    "http://localhost:1337/api/products?populate[storage][populate]=*&populate[performance][populate]=*&populate[product][populate]=*&populate[connectivity][populate]=*&populate[productImage][populate]=*&pagination[start]=0&pagination[limit]=100"
  );
  ...
  const products1 = product1Data?.map((item) => {
    const {
      ...
      attributes: {
        ...
        back_camera: {
          data: {
            attributes: {
              features: bCameraFeatures,
              resolution: bCameraResolution,
              videoRecording: bCameraVideoRecording,
            },
          },
        },
        front_camera: {
          data: {
            attributes: {
              features: fCameraFeatures,
              resolution: fCameraResolution,
              videoRecording: fCameraVideoRecording,
            },
          },
        },
        battery: {
          data: {
            attributes: { fastCharging, typeAndCapacity, wirelessCharging },
          },
        },
        sound: {
          data: {
            attributes: { earphoneJack, features: soundFeature },
          },
        },
        security: {
          data: {
            attributes: { faceLock, fingerPrint },
          },
        },
        other: {
          data: {
            attributes: { madeIn, manufacturedBy, sensors },
          },
        },
        brand: {
          data: {
    ...
    return {
      ...
      bCameraFeatures,
      bCameraResolution,
      bCameraVideoRecording,
      fCameraFeatures,
      fCameraResolution,
      fCameraVideoRecording,
      fastCharging,
      typeAndCapacity,
      wirelessCharging,
      earphoneJack,
  ...
  const products2 = product2Data?.map((item) => {
    const {
      ...
      attributes: {
        ...
        performance: {
          data: {
            attributes: { OS, chipset, processor, gpu, rams },
          },
        },
        storage: {
          data: {
            attributes: { externalSlot, roms },
          },
        },
        productImage: {
          data: {
            attributes: { width, height, hash, url },
          },
        },
      },
    } = item;
